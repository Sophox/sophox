...
from .Properties import P_KEY_ID, P_TAG_ID, P_LANG_CODE, P_INSTANCE_OF, P_SUBCLASS_OF, P_REL_ID
    ...
    def __init__(self, opts, caches: Caches, site: Site) -> None:
        ...
        for s in [
            ...
            for k, lst in list_to_dict_of_lists(
                ...
                lambda v: (v[0], sitelink_normalizer(v[1]))).items()
        ]:
            for v in s:
        ...
        self.new_items_strids = set([(v.type, v.str_id) for v in wiki_items]) - existing_items_strids
        ...
        self.wiki_items_by_id = list_to_dict_of_lists(self.caches.descriptionParsed.get(), lambda v: (v.type, v.str_id))
